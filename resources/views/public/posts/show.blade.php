...
{{--    </div>--}}
    <div class="max-w-3xl mx-auto px-4 py-10 space-y-8">
        ...
        <div class="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
          <div class="flex items-center space-x-4">
              @if($post->author->avatar)
                  <flux:avatar size="sm" circle  src=" {{ asset('storage/' . $post->author->avatar) }}" />
              @else
                  <flux:avatar size="sm" circle name="{{ $post->author->name }}" />
              @endif
        ...
        {{-- Featured image --}}
        @if($post->featured_image)
            <img src="https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=600&h=400&fit=crop" alt="{{ $post->title }}" class="rounded-2xl w-full max-h-[500px] object-cover shadow">
        @endif
