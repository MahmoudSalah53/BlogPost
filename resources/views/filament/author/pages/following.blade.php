<x-filament-panels::page>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @forelse($this->following as $followingUser)
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div class="flex items-center space-x-3">
                    <div class="relative w-12 h-12">
                        <img src="{{ $followingUser->avatar ? \Illuminate\Support\Facades\Storage::url($followingUser->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode($followingUser->name) }}"
                             alt="{{ $followingUser->name }}"
                             class="w-full h-full rounded-full object-cover">

                        @if($followingUser->role === 'author')
                            <button 
                                wire:click="toggleFollow({{ $followingUser->id }})"
                                wire:loading.attr="disabled"
                                wire:target="toggleFollow({{ $followingUser->id }})"
                                style="position: absolute; bottom: 0; right: 0; transform: translate(30%, 30%);
                                       width: 26px; height: 26px; border-radius: 9999px;
                                       background-color: {{ auth()->user()->isFollowing($followingUser) ? '#ec4899' : '#06b6d4' }};
                                       color: white; display: flex; align-items: center; justify-content: center;
                                       box-shadow: 0 4px 8px rgba(0,0,0,0.2); transition: background-color 0.2s;">

                                <svg wire:loading wire:target="toggleFollow({{ $followingUser->id }})"
                                ...
                                <span wire:loading.remove wire:target="toggleFollow({{ $followingUser->id }})">
                    ...
                    <div class="flex-1">
                        ...
                        @php
                            ...
                            $bgColor = $roleStyles[$followingUser->role]['bg'] ?? '#6b7280';
                            $textColor = $roleStyles[$followingUser->role]['text'] ?? '#ffffff';
                        @endphp

                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                              style="background-color: {{ $bgColor }}; color: {{ $textColor }}; margin-left: 10px;">
                            {{ ucfirst($followingUser->role) }}
                        </span>
                    </div>
                </div>
            </div>
