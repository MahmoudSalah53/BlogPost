 <div class="max-w-3xl mx-auto px-4 py-10 space-y-8">
     ...
     <div class="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
         <div class="flex items-center space-x-4">
             ...
             @else
                 <div
                     class="w-11 h-11 rounded-full bg-zinc-300 dark:bg-zinc-700 text-zinc-900 dark:text-white flex items-center justify-center font-semibold text-sm shadow">
     ...
     @if ($post->featured_image)
         <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }}"
             class="rounded-2xl w-full max-h-[500px] object-cover shadow">
     @endif
     {{-- Tags --}}
     @if ($post->tags->count())
