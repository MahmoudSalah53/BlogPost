...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    ...
    <flux:header sticky container class="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700">
        ...
        <flux:navbar class="-mb-px max-lg:hidden">
            <flux:navbar.item wire:navigate href="{{ route('home') }}" :current="request()->routeIs('home')">
        ...
        <flux:navbar class="me-4">
            ...
            @else
                ...
                <flux:button class="max-lg:hidden" wire:navigate href="{{ route('login') }}" variant="primary">
                    {{ __('Login') }}
                </flux:button>
            ...
            <div x-data="{ dark: $flux.dark }">
                <flux:button x-cloak x-show="!dark" x-on:click="dark = true; $flux.dark = dark" icon="moon"
                    variant="subtle" aria-label="Enable dark mode" />
                <flux:button x-cloak x-show="dark" x-on:click="dark = false; $flux.dark = dark" icon="sun"
                    variant="subtle" aria-label="Disable dark mode" />
            </div>
            {{-- sidebar toggle icon --}}
